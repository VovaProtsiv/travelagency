...
public interface RoomRepository extends JpaRepository<Room, Long> {
    ...
    @Query("SELECT r.id as id, r.name as name, r.sleeps as sleeps, r.hotel.id as hotelId, r.price as price FROM Room r WHERE id = ?1")
    RoomProjection findProjectionById(long id);

    @Query("SELECT r.id as id, r.name as name, r.sleeps as sleeps, r.hotel.id as hotelId, r.price as price FROM Room r WHERE r.hotel.id = ?1")
    List<RoomProjection> findAllProjectionByHotelId(long id);

    @Query(value = "SELECT r.id as id, r.name as name, r.sleeps as sleeps, r.hotel_id as hotelId, r.price as price " +
